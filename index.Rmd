...
effect <- select(rii, site, ESI, arid)
...
m1.arid <- glmmTMB(abun ~ Microsite + RDM + arid +(1|Region/site), family = "nbinom2", data = eph.abun)
summary(m1.arid)

m2.arid <- glmmTMB(abun ~ Microsite + RDM + arid +(1|Region), family = "nbinom2", data = eph.abun)
summary(m2.arid)

m3.arid <- glmmTMB(abun ~ Microsite + RDM + (1|Region/site), family = "nbinom2", data = eph.abun)
summary(m3.arid)

m4.arid <- glmmTMB(abun ~ Microsite + RDM + arid, family = "nbinom2", data = eph.abun)
summary(m4.arid)

m5.arid <- glmmTMB(abun ~ Microsite + RDM + arid + Region, family = "nbinom2", data = eph.abun)
summary(m5.arid)

m6.arid <- glmmTMB(abun ~ Microsite + RDM + arid + (1|site), family = "nbinom2", data = eph.abun)
summary(m6.arid)

m7.arid <- glmmTMB(abun ~ Microsite + RDM + (1|site), family = "nbinom2", data = eph.abun)


summary(m7.arid)
AIC(m1.arid, m2.arid, m3.arid, m4.arid, m5.arid, m6.arid, m7.arid)
...
m3 <- glmmTMB(abun ~ Microsite + rdm.cov + RDM + rdm.cov + (1|site), family = "nbinom2", data = eph.abun)

m4 <- glmmTMB(abun ~ Microsite + rdm.cov + RDM + rdm.cov + (1|Region/site), family = "nbinom2", data = eph.abun)
summary(m4)
...
m5 <- glmmTMB(Species ~ Microsite + RDM + rdm.cov + (1|site), family = "poisson", data = eph)
summary(m5)
...
m6 <- glmmTMB(Species ~ Microsite + RDM + rdm.cov + ESI + arid +(1|site), family = "poisson", data = eph)
summary(m6)

m7 <- glmmTMB(Species ~ Microsite + RDM*ESI + rdm.cov, family = "poisson", data = eph)
summary(m7)

AIC(m5, m6, m7)

cor.test(eph$ESI, eph$arid)
...
#need to fix this
rdm.av <- rdm %>%  group_by(site) %>% summarise(mean.rdm = mean(RDM))
...
av %>% filter(response == "abun") %>% cor.test(~mean.rdm.x + arid, data =.)
av %>% filter(response == "abun") %>% cor.test(~mean.rdm.x + ESI, data =.)



rd <- select(av, site, mean.rdm.y)
rii <- left_join(rii, rd, by = "site")
rii <- distinct(rii)

rii %>% filter(response == "richness") %>% cor.test(~arid+ average, data =.)
rii %>% filter(response == "abun") %>% cor.test(~arid+ average, data =.)



rii %>% filter(response == "rdm") %>% cor.test(~ESI+ average, data =.)
rii %>% filter(response == "rdm") %>% cor.test(~arid+ average, data =.)

library(ggplot2)

ggplot(filter(rii,response == "abun"), aes(reorder(site, arid), arid)) + geom_bar(stat="identity")
       
m1 <- lm(response ~ arid, data = filter(rii, response == "rdm"))


abun.rii <- filter(rii,response == "abun")
kruskal.test(abun.rii$region, abun.rii$average)
