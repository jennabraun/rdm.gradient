...
eph$site <- as.factor(eph$site)
eph$site <- factor(eph$site, levels = unique(eph$site[order(eph$label)]))

c <- ggplot(eph, aes(site, Species, fill = Microsite)) + geom_boxplot() + ylab("Arthropod Richness") + theme(legend.position = "bottom", legend.box = "horizontal") + scale_fill_manual(values=c("white", "gray"))
l1 <- get_legend(c)

rdm <- left_join(rdm, lbls, by = "site")
rdm$site <- factor(rdm$site, levels = unique(rdm$site[order(rdm$label)]))
...
d <- ggplot(filter(eph, abun < 350), aes(label, abun, fill = Microsite)) +  
 stat_boxplot(geom ='errorbar') + geom_boxplot() + ylab("Arthropod Abundance") + theme(legend.position = "none") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
test$label <- factor(test$label, levels = c("Pooled Sites", 1, 2, 3, 4, 5, 6, 7, 8, 9))
library(scales)
group.colors <- c("#000000", hue_pal()(9))
names( group.colors ) <- LETTERS[1:4]; group.colors


rii.plot <- ggplot(test, aes(x=response, y=average)) + geom_point(aes(response, average,  color = label), position = position_dodge(width = 0.7),  size=4)+ geom_errorbar(aes(ymin=average-ci, ymax=average + ci, color = label), width=.1, position= position_dodge(width = 0.7))  + ylim(-1, 1) + theme(axis.text.x=element_text(angle=90, vjust=.5)) + ylab("RII") + xlab("") + geom_hline(aes(yintercept=0))  + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) + scale_x_discrete(labels = c("RDM Biomass", "RDM Cover", "Arthropod Richnes", "Arthropod Abundance")) +  scale_colour_manual( values= group.colors, name = "Sites")
...
b <- ggplot(fit1$model, aes_string(x = names(fit1$model)[2], y = names(fit1$model)[1])) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red") +
  geom_label(aes(x = 0.3, y = 0.23),vjust=1, hjust = 0, 
             label = paste("Adj R2 = ",signif(summary(fit1)$adj.r.squared, 5),
...
a <- ggplot(fit1$model, aes_string(x = names(fit1$model)[2], y = names(fit1$model)[1])) + 
  ...
  geom_label(aes(x = 4, y = 3.5),vjust=1, hjust = 0, 
             label = paste("Adj R2 = ",signif(summary(fit1)$adj.r.squared, 5),
 " \nP =",signif(summary(fit1)$coef[2,4], 5))) + ylab("Mean RDM Biomass") + xlab("") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"))  + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
a
...
panel.background = element_blank(), axis.line = element_line(colour = "black"))  + theme(plot.margin=grid::unit(c(0,0,0,0), "mm")) + ylab("Mean RII RDM Biomass")
e
...
f <-  ggplot(fit1$model, aes_string(x = names(fit1$model)[2], y = names(fit1$model)[1])) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red") +
  geom_label(aes(x = 0.02, y = 0.78),vjust=1, hjust = 0, 
             label = paste("Adj R2 = ",signif(summary(fit1)$adj.r.squared, 5),
 " \nP =",signif(summary(fit1)$coef[2,4], 5))) + ylab("") + xlab("") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"))  + theme(plot.margin=grid::unit(c(0,0,0,0), "mm")) 
f
...
panel.background = element_blank(), axis.line = element_line(colour = "black"))  + theme(plot.margin=grid::unit(c(0,0,0,0), "mm")) + ylab("Mean RII Abundance")
k
