...
knitr::kable(car::Anova(m4, type = 2))
check_collinearity(m4)
m5 <- glmmTMB(Species ~ Microsite * RDM * rdm.cov + (1|Region/site), family = "poisson", data = eph)
...
region.mean<- eph.abun %>% group_by(Region, Microsite) %>% summarise(mean.RDM = mean(RDM),  mean.cover = mean(rdm.cov), mean.abun = mean(abun),  mean.rich = mean(Species))  %>% gather(var, mean, 3:6)

region.se <- eph.abun %>% group_by(Region, Microsite) %>% summarise(se.RDM = se(RDM), se.cover = se(rdm.cov), se.abun = se(abun), se.rich = se(Species)) %>% gather(var, se, 3:6)
...
ggplot(region.summ, aes(Region, mean, fill = Microsite)) + geom_bar(stat = "identity", position = "dodge") + geom_errorbar(aes(Region,  ymax = mean + se, ymin = mean - se), position = "dodge") + facet_wrap(~var, scales = "free")
